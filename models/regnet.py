...
class RR_ResNet(nn.Module):
    def __init__(self, input_channels=1, scale_range=Config().SCALE_RANGE, shift_range=Config().SHIFT_RANGE):
        super(RR_ResNet, self).__init__()
        self.scale_range = scale_range
        self.shift_range = shift_range
        self.resnet = models.resnet18(weights=None)
        self.resnet.conv1 = nn.Conv2d(input_channels*2, 64, kernel_size=7, stride=2, padding=3, bias=False)
