...
#### z_net 输出 (π / pi)
- **定义**: `z_net` 输出经过 Softmax 处理后得到的 `pi`，代表每个像素属于各个高斯分量的**变分近似后验概率** $q(z|x)$。
- **作用**: 
  - 在推断阶段，直接作为像素的类别预测概率。
  - 在训练阶段，用于计算重建损失（加权高斯似然）和 KL 散度（与先验分布对齐）。
- **性质**: 它是模型通过神经网络直接从图像特征中“学习”到的概率分布，旨在逼近真实的贝叶斯后验概率 $p(z|x)$。

#### o_net 输出 (α / alpha)
- **定义**: `o_net` 输出经过 Softplus 处理后得到的 `alpha`，代表狄利克雷分布的浓度参数。
- **作用**: 
  - 控制混合系数的先验分布 $p(\pi|\alpha)$。
  - 通过 KL 散度约束 `pi`，引入解剖学先验知识。
