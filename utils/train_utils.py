...
def forward_pass(image, label, prior, 
                 unet, x_net, z_net, o_net, reg_net, align_net,
                 config: Config, 
                 epoch, 
                 epsilon):
    ...
    alpha_mapped = alpha
    # alpha_mapped = config.PRIOR_INTENSITY * alpha           # 当前浓度参数

    # 计算Dirichlet先验分布(配准)
    affined_prior, scale_pred, tx_pred, ty_pred = compute_dirichlet_priors(alpha=alpha, 
                                                                           prior=prior, 
                                                                           reg_net=reg_net, 
                                                                           config=config,
                                                                           epoch=epoch)
